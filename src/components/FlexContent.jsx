...
const FlexContent = ({
  ifExists,
  endpoint: { heading, title, btn, text, img, url },
}) => {
  return (
    <>
      <div
      ...
      >
        <div className="max-w-lg lg:max-w-none w-full md:text-center grid items-center lg:justify-items-center">
          ...
          >
            <button
              type="button"
              className="button-theme bg-slate-900 shadow-slate-900 text-slate-100 py-1.5 px-4"
            >
              {btn}
            </button>
          </a>
        ...
        <div className="flex items-center justify-center max-w-xl relative lg:max-w-none w-full">
          <img
            src={img}
            alt={`img/${heading}`}
            className={`w-auto object-fill transitions-theme mt-8 ${
              ifExists
                ? "h-60 lg:h-56 md:h-52 sm:h-44 xs:h-36 rotate-6 hover:-rotate-12"
                : "h-72 lg:h-64 md:h-60 sm:h-48 xs:h-40 rotate-[19deg] hover:rotate-12"
            }`}
          />
        </div>
      </div>
